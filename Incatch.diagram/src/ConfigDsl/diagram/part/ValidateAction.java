...
public class ValidateAction extends Action {
	...
	*/
	public ValidateAction(IWorkbenchPage page) {
		setText(ConfigDsl.diagram.part.Messages.ValidateActionMessage);
		this.page = page;
	}
	...
	public void run() {
		...
		if (workbenchPart instanceof IDiagramWorkbenchPart) {
			...
			try {
				...
				ConfigDsl.diagram.part.ConfigDslDiagramEditorPlugin.getInstance().logError("Validation action failed", //$NON-NLS-1$
						e);
			}
		}
	}
	...
	*/
	public static void runValidation(View view) {
		try {
			if (ConfigDsl.diagram.part.ConfigDslDiagramEditorUtil.openDiagram(view.eResource())) {
			...
			ConfigDsl.diagram.part.ConfigDslDiagramEditorPlugin.getInstance().logError("Validation action failed", e); //$NON-NLS-1$
		}
	}
	...
	*/
	public static void runValidation(DiagramEditPart diagramEditPart, View view) {
		...
		ConfigDsl.diagram.providers.ConfigDslValidationProvider.runWithConstraints(txDomain, new Runnable() {
	...
	*/
	private static void validate(DiagramEditPart diagramEditPart, View view) {
		...
		if (target != null) {
			ConfigDsl.diagram.providers.ConfigDslMarkerNavigationProvider.deleteMarkers(target);
		}
	...
	*/
	private static void createMarkers(IFile target, IStatus validationStatus, DiagramEditPart diagramEditPart) {
		...
		ConfigDsl.diagram.part.ConfigDslDiagramEditorUtil.LazyElement2ViewMap element2ViewMap = new ConfigDsl.diagram.part.ConfigDslDiagramEditorUtil.LazyElement2ViewMap(
				diagramEditPart.getDiagramView(),
				collectTargetElements(rootStatus, new HashSet<EObject>(), allStatuses));
		for (Iterator it = allStatuses.iterator(); it.hasNext();) {
			IConstraintStatus nextStatus = (IConstraintStatus) it.next();
			View view = ConfigDsl.diagram.part.ConfigDslDiagramEditorUtil.findView(diagramEditPart,
	...
	private static void createMarkers(IFile target, Diagnostic emfValidationStatus, DiagramEditPart diagramEditPart) {
		...
		for (Iterator it = emfValidationStatus.getChildren().iterator(); it.hasNext();) {
			...
			if (data != null && !data.isEmpty() && data.get(0) instanceof EObject) {
				...
				View view = ConfigDsl.diagram.part.ConfigDslDiagramEditorUtil.findView(diagramEditPart, element,
						element2ViewMap);
				addMarker(diagramEditPart.getViewer(), target, view.eResource().getURIFragment(view),
	...
	*/
	private static void addMarker(EditPartViewer viewer, IFile target, String elementId, String location,
		...
		ConfigDsl.diagram.providers.ConfigDslMarkerNavigationProvider.addMarker(target, elementId, location, message,
				statusSeverity);
	}
